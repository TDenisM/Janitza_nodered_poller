...
data-i18n='modbus-contrib.label.server'></span></label> <input type='text' id='node-input-server'> </div> <!-- <hr>
    ...
    <div class="form-row">
        <label style="min-width:160px" for="node-input-emptyQueue"><i class="fa fa-th"></i> <span
        data-i18n="modbus-contrib.label.emptyQueue"></span></label>
        <input type="checkbox" id="node-input-emptyQueue" style="max-width:30px">
    </div> --> <hr> <div class='form-row'> <label style='min-width:160px' for='node-input-showStatusActivities'><i 
...
msg.payload.commandDelay || node.commandDelay </li><li>msg.payload.clientTimeout || node.clientTimeout </li><li>
msg.payload.reconnectTimeout || node.reconnectTimeout </li></ul> <p></p> <p> </p><p>Function node code examples for TCP:
</p> <code> msg.payload = { 'connectorType': 'TCP', 'tcpHost': '127.0.0.1', 'unitId': 2 } return msg </code> <code>
